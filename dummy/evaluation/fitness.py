...
import numpy as np
import math
from dummy.utils import geometric
...
''' Output only one fitness value incorporating distance and velocity information'''
class FitnessMinDistanceVelocityWeighted(Fitness):
    @property
    def min_or_max(self):
        return "min"
    ...
    @property
    def name(self):
        return "Weighted Min distance and Velocity at min distance"

    def eval(self, simout: SimulationOutput) -> Tuple[float]:

        MAX_SPEED = AVPConfig.MAX_SPEED
        MIN_SPEED = 0.1
        ...
        log.info(f"[Fitness eval] min distance = {distance}")
        log.info(f"[Fitness eval] speed = {speed}")

        weight_1 = 0.5
        weight_2 = 1 - weight_1

        fvalue = weight_1 * (distance - MIN_DISTANCE) / (MAX_DISTANCE - MIN_DISTANCE) + \
                 - weight_2 * (speed - MIN_SPEED) / (MAX_SPEED - MIN_SPEED)
